...
export function BillPaymentForm({ open, onOpenChange, defaultDate }: BillPaymentFormProps) {
  ...
  // Filter categories to show bill-related ones
  ...
  const createBillPaymentMutation = useMutation({
    mutationFn: async (data: BillPaymentFormData) => {
      return await apiRequest("POST", "/api/bill-payments", {
    ...
    onSuccess: (response, variables) => {
      ...
      toast({
        ...
        description: "Your bill payment has been successfully recorded.",
      });
      form.reset();
      setReceiptPath(null);
      setBatchInfo(null);
    ...
    onError: (error: any) => {
      toast({
        title: "Error recording bill payment",
        description: error.message || "Failed to record bill payment. Please try again.",
        variant: "destructive",
      });
    },
  });
  ...
  const handleReceiptUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (parsed.categoryHint && billCategories.length > 0) {
        const match = billCategories.find(c => 
  ...
  const handleBatchReceiptUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!files || files.length < 2) {
      toast({
        ...
        description: "Please select 2 or more invoice files to merge into one payment.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      const response = await fetch('/api/parse-receipts-batch', {
        method: 'POST',
        body: formData,
        credentials: 'include',
      });
      ...
      if (!response.ok) {
        const err = await response.json();
        throw new Error(err.message || 'Failed to parse invoices');
      }
      ...
      toast({
        ...
        description: `${parsed.receiptCount} invoices combined - Total: $${parsed.amount}`,
      });
    } catch (error: any) {
      toast({
        title: "Could not read invoices",
        description: error.message || "Try clearer files or enter details manually.",
        variant: "destructive",
      });
    } finally {
  ...
  const onSubmit = (data: BillPaymentFormData) => {
    createBillPaymentMutation.mutate(data);
  };
            ...
            <div className="flex gap-1.5">
              ...
              >
                {isParsing ? (
                  ...
                  <><Upload className="h-3.5 w-3.5" /> Invoice</>
                )}
              </Button>
              <Button
                type="button"
              ...
              >
                {isBatchParsing ? (
                  ...
                  <><Files className="h-3.5 w-3.5" /> Merge Invoices</>
                )}
              </Button>
            </div>
          </div>
        ...
        {batchInfo && (
          <div className="rounded-md border border-blue-200 bg-blue-50 dark:bg-blue-950 dark:border-blue-800 p-3 space-y-1.5">
            <p className="text-sm font-medium text-blue-800 dark:text-blue-200">
              {batchInfo.count} invoices merged into one payment
            </p>
            <div className="flex flex-wrap gap-1.5">
              {batchInfo.amounts.map((item, i) => (
                <Badge key={i} variant="secondary" className="text-xs">
                  {item.vendor || `Invoice ${i + 1}`}: ${item.amount?.toFixed(2) || '?'}
                </Badge>
              ))}
            </div>
          </div>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3">
            ...
            <div className="flex justify-end gap-2 pt-4">
              ...
              <Button
                type="submit"
                disabled={createBillPaymentMutation.isPending}
                data-testid="button-submit-bill"
              >
                {createBillPaymentMutation.isPending ? "Recording..." : "Record Payment"}
              </Button>
            </div>
          </form>
        </Form>
